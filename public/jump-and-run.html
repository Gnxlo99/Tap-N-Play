...
<body>
    <div id="game-container">
        ...
        <div id="unlock-toast">New Appearance Unlocked! ✨</div>
    ...
    <script>
        (function() {
            ...
            let obstaclePool = [];
            let currentGravity, currentObstacleSpeed, currentObstacleSpacing, currentObstacleGap;
            ...
            function resetGame() {
                ...
                obstacles.forEach(obs => obstaclePool.push(obs));
                obstacles = [];
                score = 0;
                scoreBuffer = 0;
                ...
                adBannerIframe.src = 'about:blank'; // Clear the ad
                gameState = 'Playing';
            ...
            function getGameOverMessage(score) {
                ...
                if (score <= 10) {
                    ...
                    messages = [
                        "Okay, not completely terrible.",
                        "You're starting to get it. Slowly.",
                        "That was an almost respectable attempt.",
                        "Getting warmer...",
                        "Keep trying, you might get somewhere.",
                        "I'm mildly impressed.",
                        "Hey, that wasn't half bad.",
                        "You've graduated from 'total disaster'."
                    ];
                } else { // score > 30
                    messages = [
                        "Now we're talking!",
            ...
            function showGameOverScreen(isNewHighScore, showConfetti) {
                currentScoreEl.textContent = score;
                highScoreEl.textContent = highScore;
                
                if (isNewHighScore) {
                    gameOverMessageEl.textContent = "NEW HIGH SCORE!";
                    gameOverMessageEl.style.color = '#54D77B';
                    if (showConfetti) {
                        triggerConfetti();
                    }
                } else {
                    gameOverMessageEl.textContent = getGameOverMessage(score);
                    gameOverMessageEl.style.color = ''; // Reset to default CSS color
                }
            ...
            function checkForUnlocks() {
                ...
                if (newUnlock) {
                    unlockToast.textContent = 'New Appearance Unlocked! ✨';
            ...
            function checkForEnvUnlocks() {
                ...
                if (newUnlock) {
                    unlockToast.textContent = 'New Environment Unlocked! 🎨';
            ...
            function update(deltaTime) {
                ...
                // Constant difficulty
                currentGravity = 2200;
                currentObstacleSpeed = -220;
                currentObstacleSpacing = 210;
                currentObstacleGap = 95;
                ...
                for (let i = jumpParticles.length - 1; i >= 0; i--) {
                    ...
                    p.vy += (currentGravity / 4) * deltaTime; p.x += p.vx * deltaTime; p.y += p.vy * deltaTime; p.life -= deltaTime;
                    if (p.life <= 0) jumpParticles.splice(i, 1);
                }
                
                backgroundStars.forEach(star => {
                    if(star.z > 0) { // Parallax effect
                        star.x += currentObstacleSpeed * star.z * deltaTime;
                        if (star.x < 0) star.x += CANVAS_WIDTH;
                    }
                });

                const lastObstacle = obstacles[obstacles.length - 1];
                if (!lastObstacle || lastObstacle.x < CANVAS_WIDTH - currentObstacleSpacing) {
                    const gap = currentObstacleGap;
                    ...
                    // Create obstacles using the pool
                    let topObstacle = obstaclePool.pop() || {};
                    topObstacle.x = CANVAS_WIDTH;
                    topObstacle.y = 0;
                    topObstacle.width = OBSTACLE_WIDTH;
                    topObstacle.height = obstacleY;
                    topObstacle.passed = false;
                    obstacles.push(topObstacle);

                    let bottomObstacle = obstaclePool.pop() || {};
                    bottomObstacle.x = CANVAS_WIDTH;
                    bottomObstacle.y = obstacleY + gap;
                    bottomObstacle.width = OBSTACLE_WIDTH;
                    bottomObstacle.height = CANVAS_HEIGHT - obstacleY - gap;
                    bottomObstacle.passed = false;
                    obstacles.push(bottomObstacle);
                }

                obstacles.forEach(o => {
                    o.x += currentObstacleSpeed * deltaTime;
                ...
                if (isGameOver) {
                    ...
                    // Load the banner ad into the iframe
                    adBannerIframe.src = `banner.html?t=${new Date().getTime()}`;
                    ...
                    showGameOverScreen(isNewHighScore, isNewHighScore && oldHighScore > 0);
            ...
            function renderPlayerCustomizeModal() {
                ...
                APPEARANCES.forEach((appearance, index) => {
                    ...
                    if (!isUnlocked) {
                        const lockIcon = document.createElement('span'); lockIcon.className = 'lock-icon'; lockIcon.textContent = '🔒'; item.appendChild(lockIcon);
                    } else {
